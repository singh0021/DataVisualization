...
map.beer <- plot_usmap(data = c.beer, values = "Aggtotal", region = "states", labels =TRUE) +
  scale_fill_gradientn(name = "Million barrels", colors = light_Dark.color) +
  theme(plot.background   =element_rect(fill = "orange"),
        ...
        legend.title = element_text(size = rel(1.2),
                                    family = "Bahnschrift",
                                    color="black"))+
  labs(title = "Total Beer Production By US States", subtitle = "YEAR: {current_frame}")+transition_manual(year)
